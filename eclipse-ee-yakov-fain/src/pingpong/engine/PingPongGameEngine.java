	...
	public void run(){   
		...
		while (true) {  
			if(ballServed){ // если мяч движется!!!!  !
				// Шаг 0.5 Мяч движется вверх или вниз
				if (movingUp && ballY <= BALL_MIN_Y + TABLE_BORDER - BALL_INCREMENT){
					movingUp = false;
					verticalSlide = -verticalSlide;
				} else if (!movingUp && ballY >= BALL_MAX_Y - TABLE_BORDER*2 + BALL_INCREMENT){
					movingUp = true;
					verticalSlide = -verticalSlide;
				}
				...
				if (movingLeft && ballX > BALL_MIN_X){ 
					...
					// Может отскочить?     
					if (ballX <= COMPUTER_RACKET_X + BALL_SIZE && canBounce){ 
						movingLeft=false;          
					} 
				}
				...
				// Шаг 3.5, Перемещать ракетку компьютера вверх или вниз, 
				// чтобы блокировать мяч 
				if (kidRacket_Y < ballY && kidRacket_Y < TABLE_BOTTOM){ 
					kidRacket_Y +=RACKET_INCREMENT; 
				}else if (kidRacket_Y > TABLE_TOP){ 
					kidRacket_Y -=RACKET_INCREMENT; 
				}
				table.setKidRacket_Y(computerRacket_Y);
